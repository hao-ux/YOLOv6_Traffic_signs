...
def get_yolo_loss(input_shape, num_layers, num_classes, iou_type='siou'):
    def yolo_loss(args):
        #-----------------------------------------------#
        ...
        return get_losses(x_shifts, y_shifts, expanded_strides, outputs, labels, num_classes, iou_type=iou_type)
    return yolo_loss


def get_losses(x_shifts, y_shifts, expanded_strides, outputs, labels, num_classes, iou_type='siou'):
    #-----------------------------------------------#
    ...
    def loop_body(b, num_fg, loss_iou, loss_obj, loss_cls):
        ...
        num_fg      += num_fg_img
        if iou_type == 'siou':
            loss_iou    += K.sum(1 - box_siou(reg_target, tf.boolean_mask(bboxes_preds_per_image, fg_mask)))
        elif iou_type == 'ciou':
            loss_iou    += K.sum(1 - box_ciou(reg_target, tf.boolean_mask(bboxes_preds_per_image, fg_mask)))
        else:
            ValueError('no select iou_type')
...
def box_siou(b1, b2):
    ...
    iou = inter / union
    cw = K.maximum(b1_x2, b2_x2) - K.minimum(b1_x1, b2_x1)
    ch = K.maximum(b1_y2, b2_y2) - K.minimum(b1_y1, b2_y1)
    s_cw = (b2_x1 + b2_x2 - b1_x1 - b1_x2) * 0.5
    s_ch = (b2_y1 + b2_y2 - b1_y1 - b1_y2) * 0.5
    sigma = K.pow(s_cw ** 2 + s_ch ** 2, 0.5)
    sin_alpha_1 = K.abs(s_cw) / sigma
    sin_alpha_2 = K.abs(s_ch) / sigma
    threshold = pow(2, 0.5) / 2
    sin_alpha = tf.where(sin_alpha_1 > threshold, sin_alpha_2, sin_alpha_1)
    angle_cost = K.cos(tf.experimental.numpy.arcsin(sin_alpha) * 2 - math.pi / 2)
    rho_x = (s_cw / cw) ** 2
    rho_y = (s_ch / ch) ** 2
    gamma = angle_cost - 2
    distance_cost = 2 - K.exp(gamma * rho_x) - K.exp(gamma * rho_y)
    omiga_w = K.abs(w1 - w2) / K.maximum(w1, w2)
    omiga_h = K.abs(h1 - h2) / K.maximum(h1, h2)
